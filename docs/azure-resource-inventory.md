...
Verified 13 September 2026. Owner: Osman Orka, independent pilot. Subscription identifiers and full management responses remain private under gitignored `.local/`.
...
Rates were refreshed in **AUD** from the public [Azure Retail Prices API](https://prices.azure.com/api/retail/prices), filtering Australia East `Foundry Models`. The selected public meter records and the delta from the 12 September snapshot are in [azure-price-snapshot.json](azure-price-snapshot.json). The new API precision changed fourth-to-sixth decimal digits but left the planned 72-case integer micro-AUD maximum reservation unchanged, below the 10% stop threshold. Stored GPT-5 rates use six decimals and are rounded upward at that precision.
...
GPT-5 units are 1M tokens. The current API exposes the embedding meter as 0.0000 AUD per 1K at its displayed precision, so the gateway removed that price pin rather than treating zero as billable truth. Any live embedding request therefore hard-stops as an unknown price. No invented FX rate is used. Retail estimates can differ from billing-agreement prices, currency updates and taxes; the adapter refuses pins older than 31 days.
