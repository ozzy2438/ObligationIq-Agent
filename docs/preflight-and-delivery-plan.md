...
Updated 13 September 2026, Australia/Melbourne. The Phase 0 decisions are retained below; later outcomes and superseding ADRs are identified explicitly.
...
The [public build brief](build-brief.md) excludes Section 11 entirely. The original private file is unchanged. Phase 0 was completed and stopped at its requested boundary. The owner subsequently authorised continuation; [Phase 1](phase-1-corpus.md) now adds the local regulatory corpus without a customer population or paid Azure workload.
...
Read-only catalog queries confirmed the intended cheap, strong and embedding tiers in `australiaeast`. Microsoft retail-price records supply the pinned AUD rates; see [the inventory](azure-resource-inventory.md). A bounded S0 Azure OpenAI account and GPT-5 nano/mini deployments were later provisioned and evaluated. No Foundry project, Databricks warehouse, managed storage, search service or API Management instance was provisioned.
...
6. Release only confirmed pre-dispatch failures. ADR-008 later added conservative recovery for genuinely ambiguous responses; ADR-009 separates complete pre-execution 429 capacity rejections and releases them.
7. The PII interface is invoked. The Phase 0 stub was replaced by the active structured boundary in ADR-003. Entra authentication and deployment identity checks were validated in the bounded Phase 7 run.

## Disagreements and deliberate boundaries

- Blanket “release on failure” is unsafe after timeouts or crashes. Genuine ambiguity now settles at maximum under ADR-008; no auto-expiry or ledger reset command is supplied.
...
- The two text models are now deployed and evaluated, but no Foundry orchestration or Azure-hosted application is claimed.
- SQLite implements the ledger/log/cache and local corpus. Local CPU embeddings, Delta register, Unity Catalog OSS, MLflow, controls, agents and MCP remain local. Phase 7 completed a four-arm evaluation; Phase 8 packaged the architecture and limitations. The current status is in the README.

## Verification

Run the commands in [the reproduction guide](reproduction.md). CI denies unintended network/model use and verifies population, controls, risk, agents, evaluation evidence, cost-model rendering and import/data boundaries.
